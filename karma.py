...
regex_upvote = r"^((?i)\+|\+\+|\+1|thx|tnx|ty|thank you|thanx|thanks|pro|cool|good|awesome|👍)$"
regex_downvote = r"^(\-|\-\-|\-1|👎)$"
...
async def start(_, message):
    await message.reply_text(
        "Hey, I'm A reputation Bot, There will be no further explanation.\nThere will just be reputation.\nYou Can increase or decrease rep of Someone Using Me, Join @SwiftiesWorld!"
    )
...
async def upvote(_, message):
    ...
    await message.reply_text(
        f'Increased rep of {user_mention} By 1 \nTotal Points: {members[f"{user_id}"]}'
    )
...
async def downvote(_, message):
    ...
    with open(filename, "w") as f3:
        f3.write(json.dumps(members))
    await message.reply_text(
        f'Decreased rep Of {user_mention} By 1 \nTotal Points: {members[f"{user_id}"]}'
    )
